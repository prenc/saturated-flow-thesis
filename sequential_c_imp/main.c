...
void count_q(float *pDouble, int i, int j, int i1, int j1);
...
void transition_function(int i, int j) {
    ...
    Q -= ca.Source[i][j];
    float area = CELL_SIZE_X * CELL_SIZE_Y;
    float ht1 = (Q * delta_t_) / (area * ca.Sy[i][j]);
...
void count_q(float *pDouble, int i, int j, int i1, int j1) {
    float diff_head = ca.head[i1][j1] - ca.head[i][j];
    float tmp_t = ca.K[i1][j1] * THICKNESS;
